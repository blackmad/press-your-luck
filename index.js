...
function init() {
  ...
  let saveImage = () => {
    let image = renderer.domElement.toDataURL("image/jpeg", 0.6);
  ...
  const makeDatGui = () => {
    // Dat gui
    var gui = new dat.GUI();
    gui.add(uniforms.u_scale, "value", 0.1, 10).name("Scale");
    gui.add(uniforms.u_blades, "value", 1, 20).name("Blades");
    gui.add(config, "mouse_pos").name("move on mouse");
    gui
      .add(uniforms.u_pos.value, "x", 0, 1)
      .name("x")
      .listen()
      .step(0.01);
    gui
      .add(uniforms.u_pos.value, "y", 0, 1)
      .name("y")
      .listen()
      .step(0.01);
    gui
      .add(uniforms.u_rotation, "value", -Math.PI, Math.PI)
      .name("texture rotation")
      .step(0.01);
    gui
      .add(uniforms.u_rotation_k, "value", -Math.PI, Math.PI)
      .name("rotation")
      .step(0.01);
    ...
    gui
      .add(uniforms.u_droste_rad1, "value", -1, 2)
      .name("droste radius 1")
      .step(0.01);
    gui
      .add(uniforms.u_droste_rad2, "value", -1, 2)
      .name("droste radius 2")
      .step(0.01);
  ...
  addListeners(container, "drop", e => {
    ...
    if (file.type === "image/png" || file.type === "image/jpeg") {
      ...
      reader.addEventListener("loadend", e => {
        ...
        console.log(dataurl);
        loader.load(dataurl, function do_something_with_texture(tex) {
...
function onload() {
  ...
  if (true || isIOS()) {
    ...
    navigator.mediaDevices
      ...
      .then(gotStream)
      .catch(s => window.alert(s))
      .finally(s => console.log("wtf"));
  } else {
